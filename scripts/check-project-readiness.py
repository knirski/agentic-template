...
def read_text(path: Path, findings: list[Finding]) -> str | None:
    try:
        ...
        findings.append(Finding("READINESS_MISSING_FILE", path, "file is missing", "restore the required file"))
    except (OSError, UnicodeError) as exc:
        findings.append(Finding("INTERNAL_READINESS_ERROR", path, f"cannot read file ({exc})", "fix the file and rerun validation"))
    return None
...
def check_prd(findings: list[Finding]) -> None:
    ...
    if PRD_MARKER in text:
        findings.append(Finding("READINESS_PRD_MARKER", PRD, "template replacement marker remains", "replace the marked PRD with product requirements"))
    if "This file is authoritative for the Agentic Delivery Template" in text:
        findings.append(Finding("READINESS_PRD_BOILERPLATE", PRD, "template-source boilerplate remains", "replace the introductory template contract"))

    headings = [(n, m.group(1).strip()) for n, line in lines if (m := re.match(r"^##\s+(.+?)\s*$", line))]
    positions: list[int] = []
    for title in REQUIRED_HEADINGS:
        matches = [n for n, heading in headings if heading == title]
        if not matches:
            findings.append(Finding("READINESS_PRD_HEADING_MISSING", PRD, f"required heading '## {title}' is missing", f"add exactly one '## {title}' section"))
        else:
            if len(matches) > 1:
                findings.append(Finding("READINESS_PRD_HEADING_DUPLICATE", PRD, f"heading '## {title}' appears {len(matches)} times", f"keep one '## {title}' section"))
            positions.append(matches[0])
    if positions and positions != sorted(positions):
        findings.append(Finding("READINESS_PRD_HEADING_ORDER", PRD, "required headings are out of order", "reorder the required level-two sections"))

    req_section = next((n for n, heading in headings if heading == "Requirements"), None)
    if req_section is None:
        return
    req_end = next((n for n, heading in headings if n > req_section), len(text.splitlines()) + 1)
    ...
    for number, line in req_lines:
        ...
        if match:
            ...
            if digits == "000":
                findings.append(Finding("READINESS_REQUIREMENT_ID", PRD, f"requirement {identifier} has an invalid identifier", "use an identifier from REQ-001 through REQ-999"))
            if identifier in declarations:
                findings.append(Finding("READINESS_REQUIREMENT_DUPLICATE", PRD, f"requirement {identifier} is declared more than once", "keep one declaration for the identifier"))
            declarations[identifier] = (number, title)
            if title == "":
                findings.append(Finding("READINESS_REQUIREMENT_TITLE", PRD, f"requirement {identifier} has an empty title", "add a non-empty requirement title"))
            continue
        malformed = re.match(r"^###\s+(REQ-[^:]+):", line)
        if malformed:
            findings.append(Finding("READINESS_REQUIREMENT_ID", PRD, "requirement heading has an invalid identifier", "use an identifier from REQ-001 through REQ-999"))
    if not declarations:
        findings.append(Finding("READINESS_REQUIREMENT_MISSING", PRD, "no requirement declaration exists", "add at least one ### REQ-001: Title declaration"))
    for identifier, (line_no, _) in declarations.items():
        next_heading = next((n for n, line in req_lines if n > line_no and line.startswith("### ")), req_end)
        body = [line.strip() for n, line in req_lines if line_no < n < next_heading and line.strip()]
        if not body:
            findings.append(Finding("READINESS_REQUIREMENT_BODY", PRD, f"requirement {identifier} has no body", "add explanatory and acceptance content below the requirement heading"))
...
def check_hook(findings: list[Finding]) -> None:
    ...
    if text is not None and HOOK_SENTINEL in text:
        findings.append(Finding("READINESS_HOOK_SENTINEL", HOOK, "unconfigured hook sentinel remains", "replace the stub with project validation commands"))


def main(argv: list[str]) -> int:
    if argv:
        print("INTERNAL_READINESS_ERROR: scripts/check-project-readiness.py: unexpected arguments; next: run it without arguments", file=sys.stderr)
        return 2
    ...
    try:
        ...
        print(f"INTERNAL_READINESS_ERROR: repository: {exc}; next: fix the checker input or implementation", file=sys.stderr)
        return 2
