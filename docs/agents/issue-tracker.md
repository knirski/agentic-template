...
- For Spec-backed work, `plan.json` owns task decomposition and dependencies. Each task's `execution` field either stores inline task state or points that task to the configured tracker issue.
