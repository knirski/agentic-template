...
FIXTURE_TEMPLATES: dict[str, bytes] = {
    "tooling": (
        ...
        b"rygor:if:strict:begin\n[tool.ruff]\nstrict = true\n"
        b"rygor:if:strict:end\n"
    ),
    "ci": b"name: ci\njobs:\nrygor:slot:ci-jobs\nrygor:slot:release-job\n",
    "releaserc": b'{"branches": [rygor:value:channel]}\n',
    "core-job": b"  core-check:\n    runs-on: ubuntu-latest",
    "extra-job": b"  extra:\n    runs-on: ubuntu-latest",
    "audit-job": b"  audit:\n    runs-on: ubuntu-latest",
    "release-job": (
        b"  release:\n    runs-on: ubuntu-latest\n"
        b"    if: github.ref == rygor:value:main-branch"
    ),
    "doc-fragment": (
        b"# Capabilities\n\nFrozen expansion for profile **rygor:value:project-name**."
    ),
...
def test_unknown_substitution_marker_is_rejected() -> None:
    store, content_ids = fixture_blobs()
    store, unknown_id = intern(store, b'[project]\nname = "rygor:value:nope"\n')
    render_input = make_render_input(store, content_ids)
    core = render_input.core.model_copy(
        update={
            "artifacts": (
...
def test_value_marker_prefix_is_not_partially_substituted() -> None:
    ...
    assert result == Ok(b"name = rygor:value:repo_url\n")
...
def test_empty_slot_renders_its_marker_empty() -> None:
    ...
    assert b"rygor:slot:" not in rendered[".github/workflows/ci.yml"]


def test_missing_slot_marker_in_owner_artifact_is_rejected() -> None:
    store, content_ids = fixture_blobs()
    store, partial_ci_id = intern(store, b"name: ci\njobs:\nrygor:slot:ci-jobs\n")
    render_input = make_render_input(store, content_ids)
    core = render_input.core.model_copy(
        update={
            "artifacts": (
...
def test_unknown_slot_marker_is_rejected() -> None:
    store, content_ids = fixture_blobs()
    store, ghost_id = intern(store, b'[project]\nname = "x"\nrygor:slot:ghost\n')
    render_input = make_render_input(store, content_ids)
    core = render_input.core.model_copy(
        update={
            "artifacts": (
...
def test_unbalanced_optional_section_is_rejected() -> None:
    store, content_ids = fixture_blobs()
    store, unbalanced_id = intern(store, b"[project]\nrygor:if:strict:begin\n")
    render_input = make_render_input(store, content_ids)
    core = render_input.core.model_copy(
        update={
            "artifacts": (
...
def test_missing_substitution_value_is_rejected() -> None:
    store, content_ids = fixture_blobs()
    store, broken_body = intern(store, b"  broken:\n    value: rygor:value:ghost\n")
    definitions = fixture_capabilities(content_ids)
    definitions["semantic-release"] = definitions["semantic-release"].model_copy(
        update={
            "contributions": (
...
def test_optional_section_markers_at_line_and_file_boundaries() -> None:
    ...
    store, inline_id = intern(
        store,
        b"prefix rygor:if:strict:begin mid rygor:if:strict:end suffix",
    )
    core = render_input.core.model_copy(
        update={
            "artifacts": (
...
def test_maintenance_substitutions_render_retained_paths() -> None:
    result = apply_substitutions(
        b"status = rygor:value:status\nretained = rygor:value:kept\n",
        (
            SubstitutionDefinition(
                name="status",
...
def test_optional_section_missing_substitution_value_is_rejected() -> None:
    store, content_ids = fixture_blobs()
    store, template_id = intern(store, b"rygor:if:ghost:begin\nx\nrygor:if:ghost:end\n")
    render_input = make_render_input(store, content_ids)
    core = render_input.core.model_copy(
        update={
            "artifacts": (
...
def test_value_marker_without_declared_context_is_rejected() -> None:
    store, content_ids = fixture_blobs()
    store, template_id = intern(store, b"name = rygor:value:anything\n")
    render_input = make_render_input(store, content_ids)
    core = render_input.core.model_copy(
        update={
            "artifacts": (
