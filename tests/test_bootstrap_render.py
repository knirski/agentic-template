...
FIXTURE_TEMPLATES: dict[str, bytes] = {
    ...
    "release-job": (
        b"  release:\n    runs-on: ubuntu-latest\n"
        b"    if: github.ref == 'refs/heads/agentic-template:value:main-branch'"
    ),
    "doc-fragment": (
...
def test_default_branch_flows_through_rendered_output() -> None:
    ...
    assert b"refs/heads/trunk" in trunk[".github/workflows/ci.yml"]
...
def test_cardinality_violations_are_rejected() -> None:
    store, content_ids = fixture_blobs()
    definitions = fixture_capabilities(content_ids)
    definitions["semantic-release"] = definitions["semantic-release"].model_copy(
        update={
...
def test_empty_exactly_one_slot_is_rejected() -> None:
    store, content_ids = fixture_blobs()
    definitions = fixture_capabilities(content_ids)
    definitions["semantic-release"] = definitions["semantic-release"].model_copy(
...
def test_empty_optional_slot_removes_its_marker() -> None:
    store, content_ids = fixture_blobs()
    core = fixture_core(content_ids).model_copy(
        update={
            "slots": (
...
def test_encode_scalar_uses_declared_context() -> None:
    ...
    assert encode_scalar("value", "json") == '"value"'
    assert encode_scalar(True, "json") == "true"
    assert encode_scalar("it's", "shell") == "'it'\\''s'"
    assert encode_scalar(False, "shell") == "false"
